...
object CassandraWriteResultsListener extends MetricsSupport {
  protected val LOGGER: Logger = LoggerFactory.getLogger(CassandraWriteResultsListener.getClass)
  protected val writeFailures: Meter = metricRegistry.meter("cassandra.write.failure")
  protected val writeWarnings: Meter = metricRegistry.meter("cassandra.write.warning")
}

class CassandraWriteResultsListener(asyncResult: ResultSetFuture,
                                    timer: Timer.Context) extends Runnable {

  import CassandraWriteResultsListener._
